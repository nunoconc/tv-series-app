...
### Environment Variables
- **REACT_APP_OMDB_KEY** - The URL to access the OMDB API
- **REACT_APP_OMDB_API** - The API key to access the OMDB API
- **REACT_APP_GRAPHQL_API_URL** - The URL to access the GraphQL API
- **REACT_APP_GRAPHQL_WS_URL** - The URL to access the GraphQL API with Websockets
- **REACT_APP_API_KEY** - The API key to access the GraphQL API both http and websocket
...
### Context
This folder contains the context that is used to manage the state of the application.
In this case for the singleton service OMDB.

### Graphql
This folder contains the graphql queries, mutations and subscriptions that are used to interact with the API.

### Pages
This folder contains the main pages of the application. List of series and the detail of the series.

### Router
This folder contains the router configuration of the application.
Plus the subscription async navigations.

### Services
This folder contains the services that are used to interact with both graphql and OMDB APIs.

### Types
This folder contains the types that are used across the application.
...
### Cache & Storage
- **Apollo Client** cache to store the data that is fetched from the API. (In memory cache)
- **Axios Cache** to store the data and images that is fetched from the OMDB API. (In local storage)

### Screen Size
- **Tailwind CSS** helps to make the application responsive to different screen sizes.\
However was needed to handle the list to fit to smaller screens.


## Deployment
This application is deployed in the gh-pages of the repository.\
GitHub can access it and deploy the application in the following URL:\
[TV Series App](https://nunoconc.github.io/tv-series-app/)
